...
if (isset($_POST['usuario']) && isset($_POST['psw'])) {
    ...
    if (count($resultado) > 0) {
        ...
        $favs = $pdo -> prepare("SELECT * FROM favoritos WHERE id_Cliente = 
        (SELECT id_cliente FROM Clientes where id_Usuario = :idUser LIMIT 1) ");
        $favs -> bindParam(':idUser', $resultado['id_Usuario'], PDO::PARAM_STR);
        $favs ->execute();
        $numFavs = $favs -> fetchAll(PDO::FETCH_ASSOC);
        $_SESSION['Favs'] = count($numFavs);
